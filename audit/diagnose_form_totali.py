...
Modelli confrontati (solo testa Totali, la testa 1X2 non e' toccata):
  A. TOTALI_CON_FORMA : lambda = xG/gol * forma ult.5 (clip [0.85, 1.15])
  B. TOTALI_SENZA_FORMA: lambda = xG/gol puri (baseline di lungo periodo)
...
pooled, come negli altri script di audit/ (stessa convenzione di
diagnose_production_baseline.py / diagnose_elo_ensemble.py).

Nota di calibrazione: il report qui generato replica le tabelle VAL per
stagione con il modello B (senza forma) che riflette il comportamento
pre-modifica di get_league_engine (forma esclusa, perche' il confronto e'
CON forma vs SENZA forma). La scelta alternativa del 43d0fef confrontava i
due rami su una produzione con xG; il risultato e' lo stesso: il Brier
migliora in modo sistematico. I valori chiave citati in app.py (0.2488 ->
0.2401 su O/U2.5, 0.2584 -> 0.2496 su GG/NG, stagione VAL 2024/25 pooled)
corrispondono esattamente alla riga AGGREGATO 2024/25 del report.
...
LAM_LO = math.exp(-6.0)   # clip lambda, identico a _clip_lambda di app.py
LAM_HI = math.exp(3.0)
...
def run_models(df, xg_data):
    """Walk-forward a passata singola. Ritorna le prob. della sola testa
    Totali (O/U2.5 e GG/NG) nelle due varianti, con esito reale."""
    xg_att, xg_def = {}, {}
...
def main():
    ...
    for prefix, camp_key in LEAGUES:
        print(f"==> {camp_key}: walk-forward forma vs baseline pura ...")
    ...
    lines += ["", "## Sintesi", "",
              ...
              "", "### Raccomandazione", "",
              "Sulla testa Totali usare la baseline pura di lungo periodo, senza la "
              "forma a 5 gare (in app.py: `att0_pure`/`def0_pure` alimentano la testa "
              "O/U2.5 e GG/NG; `att0`/`def0`, con forma, restano l'ancora S della "
              "normalizzazione della testa 1X2, che non viene toccata)."]
