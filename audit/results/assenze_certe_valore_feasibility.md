...
| Infortuni pre-match | **Prospettico si'; storico NO a costo zero da infrastruttura automatizzata** (misurato): WhoScored serve blocco Cloudflare 403 agli IP datacenter su ogni via tentata | il dato esiste sulla fonte (preview passate persistono, evidenza al 2021) ma e' irraggiungibile da cloud; storico possibile solo da client non bloccato, o fonte alternativa/pagamento | WhoScored via soccerdata §4 |
...
|---|---|---|---|
| Serie A | 5a ammonizione → 1 giornata (art. 19 c.9 CGS, in vigore dal 2015/16) | progressivi, confermati sul testo ufficiale FIGC (files.figc.it): 5, poi 4, 4, 3, 2, poi ogni ammonizione (squalifiche alla 5a, 9a, 13a, 16a, 18a; dal 19° giallo in poi ogni ammonizione squalifica) | cartellini di Coppa Italia/Europa non comunicanti con la Serie A |
...
- **Campionamento REALE predisposto ed eseguito sul perimetro esatto (non una partita sola)**: il workflow `.github/workflows/whoscored_missing_sample.yml` (stesso escamotage GitHub Actions della Parte A) esegue `audit/whoscored_missing_players_sample.py` su 5 leghe × stagioni 2223–2627, campione stratificato per cella (8 partite a quantili equispaziati), con stati espliciti OK_ROWS / OK_EMPTY_SECTION / SECTION_MISSING / BLOCKED / FAILED, copertura % per lega/stagione/cella, e due vie d'accesso decise da sonde refertate: (a) HTTP puro con TLS impersonation `curl_cffi` sugli stessi endpoint di soccerdata (pagina torneo → select stagioni, pagina stagione → stage, `/tournaments/{stage}/data/?d=YYYYMM` per il calendario JSON, `/Matches/{id}/Preview` con la stessa xpath di `read_missing_players`), (b) Selenium `uc=True` con attesa maggiorata. Eseguito 4 volte sul branch (run 35474067133 → 35475174937; referti come commenti sul PR + artifact).
- **Risultato MISURATO (run finale 35475174937)**: **HARD_BLOCK**. Sonda HTTP: **403** su homepage e preview storica 2021, 4/4 tentativi, pagina di blocco, nessun dato. Via Selenium: pagina Cloudflare "Attention Required! / Sorry, you have been blocked" su 9/9 tentativi (home, pagina lega, preview storica; headless e headed; riprovando a distanza), senza mai `allRegions`, select stagioni ne' `missing-players`. La sonda HTTP dimostra che **non e' rilevazione del browser/fingerprint: e' un blocco IP/ASN** — Cloudflare rifiuta gli intervalli datacenter dei runner GitHub (la sandbox di lavoro e' ugualmente bloccata). La % di copertura nel perimetro quindi **non e' stimabile da infrastruttura cloud a costo zero**: il dato esiste (le preview passate persistono, v. evidenza sopra) ma la via d'accesso gratuita e automatizzata che ha funzionato per PPDA/deep/giocatori qui NON funziona. Nota implementativa: la pagina di blocco non e' riconosciuta dai marcatori anti-CAPTCHA di soccerdata 1.9.1, che la scambia per dati vuoti — da trattare esplicitamente in qualunque implementazione futura.
...
3. **campionamento reale ESEGUITO** (`audit/whoscored_missing_players_sample.py` + workflow `.github/workflows/whoscored_missing_sample.yml`, stesso escamotage GitHub Actions della Parte A): verdetto misurato **HARD_BLOCK** (blocco Cloudflare IP/ASN su ogni via, §4) — la copertura % storica non e' misurabile a costo zero dal cloud. Il workflow resta eseguibile (dispatch manuale): se in futuro l'esecuzione avviene da un client/IP non bloccato, produce automaticamente la stima di copertura per cella con gli stati espliciti previsti;
...
- Le regole del §2 sono verificate su fonti pubbliche/regolamenti citati; la progressione Serie A e' confermata sul testo ufficiale CGS art. 19 (files.figc.it); resta buona pratica ricontrollare il testo vigente al momento dell'implementazione per eventuali modifiche nel frattempo intervenute;
- il caso "persistenza preview storiche" di WhoScored e' provato dalla documentazione ufficiale per il 2021; il campionamento nel nostro perimetro e' stato eseguito (4 run GitHub Actions) ma la fonte risulta HARD_BLOCK per IP/ASN da infrastruttura cloud (§4): la copertura storica reale non e' misurabile da questa via, e l'acquisizione automatizzata a costo zero non e' fattibile senza un client non bloccato o una fonte alternativa;
...
- Reset/azzeramenti: testo ufficiale CGS art. 19 c.9-10 (files.figc.it, allegati comunicati FIGC: progressione cicli 5,4,4,3,2,poi ogni ammonizione; ammonizioni inefficaci solo a fine stagione o al trasferimento in Lega diversa — nessun azzeramento infrastagionale); RFEF art. 112 testo integrale con modifica 2019/20 (sport.es/7579111, as.com 2019-08-01, sport.es/8105445, approvazione CSD 19/05/2020: esenzione ultima giornata e cicli Copa azzerati dopo i quarti); PL scadenze soglie 19a/32a partita — nytimes.com/athletic/3611024 e tabella cutoff nytimes.com/athletic/6000618.
- Casi campione: theanalyst.com "Most Cards in a Premier League Season" (Palhinha 14 gialli 2022/23); **Andreas Pereira** (Fulham 2022/23, 5o giallo dopo la 19a partita → nessuna squalifica: nytimes.com/athletic/4095181, 16/01/2023); **Cheryshev** (Real Madrid, Copa 2015: squalifica ereditata scontata in campo → esclusione del Real, caso che ha motivato l'art. 112.4); fantamaster.it (Gatti/McKennie, 2023/24); futbolfantasy.com (Catena 2026); magazin.comunio.de (Olise/Diks 2026); lfp.fr "Commission de discipline 17/04/2024" (Lees-Melou) e decisioni LFP con formula "troisieme avertissement dans une periode incluant 10 rencontres de competition officielle (Ligue 1, Coupe de France, Trophée des Champions)".
- Blocco WhoScored misurato: run GitHub Actions di questo branch 35474067133, 35474219305, 35474443239, 35474617636, 35475174937 (referti nei commenti del PR e artifact): HTTP 403 con curl_cffi e pagina Cloudflare "Sorry, you have been blocked" con Selenium uc=True su ogni endpoint, homepage/pagine lega/preview storica 2021.
