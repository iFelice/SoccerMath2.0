...
Walk-forward no-leakage, 5 leghe, VALIDATION 2024/25 + TEST 2025/26. Poisson = testa 1X2 di produzione (xG + forma + mercato, somma normalizzata alla base con forma att0/def0). Elo = sequenziale K=24, home advantage per lega, pareggio gaussiano (stessa formula del backtest in-app). Ensemble = w*Poisson + (1-w)*Elo. In produzione (Value Bets) w = 0.6.
