...
Rimuovere `form_att`/`form_def` dal calcolo di `att0`/`def0` in `get_league_engine()` (app.py): la testa Totali deve usare la baseline pura di lungo periodo. La forma resta nella testa 1X2 (`att`/`def`).
