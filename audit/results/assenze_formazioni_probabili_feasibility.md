...
- **soccerdata 1.9.1** (gia' pinnata nel progetto): espone lettori di formazioni **ufficiali** storiche — `FBref.read_lineup`, `ESPN` (lineups), `Sofascore` (lineups) [17] — e, via WhoScored, `read_missing_players()`: "lista di giocatori infortunati e squalificati prima di ogni partita" [18]. Quest'ultimo e' l'unico canale **pre-match sulle assenze** dentro la libreria gia' in uso, ma: riguarda solo partite correnti/imminenti (nessun archivio storico), e WhoScored e' una fonte non ufficiale (scraping, ToS restrittivi): da trattare come eventuale integrazione opportunistica, non come fonte primaria.
