...
In app.py (tab Value Bets) la confidence 1X2 e' `0.6 * poisson + 0.4 * elo`.
Questo script misura, walk-forward no-leakage su 5 leghe (VALIDATION 2024/25 +
TEST 2025/26), Brier e LogLoss multiclasse 1X2 di:

  - POISSON       : lambda di produzione (xG primario + forma ult.5 + mercato,
                    somma normalizzata come la testa 1X2 a Due Teste)
...
def main():
    ...
    lines = ["# Ensemble Poisson + Elo sulla testa 1X2", "",
             ...
             "Poisson = lambda di produzione (xG + forma + mercato, somma normalizzata). "
             "Elo = sequenziale K=24, home advantage per lega, pareggio gaussiano "
             "(stessa formula del backtest in-app). Ensemble = w*Poisson + (1-w)*Elo. "
             "In produzione (Value Bets) w = 0.6.", ""]
    win_counts = {name: 0 for name, _ in models}; total = 0
