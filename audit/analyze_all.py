"""
analyze_all.py — Analisi delle 5 leghe (stessa struttura tabelle di analyze.py per Serie A).

Replica IDENTICA di analyze.py (Serie A) estesa alle altre 4 leghe. Nessun commento
interpretativo aggiunto: produce, per ogni lega, le stesse tabelle stagione per stagione
(Tabella 1 modelli, Tabella 2 B365 vs Average Market, Over/Under, GG/NG solo calibrazione,
piu' la calibrazione dettagliata SoccerMath esito '1' sul test 2025/26).

L'unica cosa che cambia rispetto ad analyze.py e':
  - il caricamento dei dati (prefisso CSV + chiave lega in LEAGUE_HOME_ADVANTAGE)
"""
import os
import sys
import numpy as np
...
def analyze_league(prefix, camp_key):
    ...
    for label, season in seasons.items():
        ...
        print(f"\n{'='*70}\n{camp_key.upper()} — {label}  (N={len(sub)} partite)\n{'='*70}")
        if sub.empty:
    ...
    if not test.empty:
        print(f"\n{'='*70}\nCalibrazione dettagliata — SoccerMath, esito '1' (casa vince) — TEST 2025/26 — {camp_key.upper()}\n{'='*70}")
