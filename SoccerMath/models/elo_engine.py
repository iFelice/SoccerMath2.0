...
# Parametri del Modello Elo
DEFAULT_INITIAL_RATING = 1500.0
HOME_ADVANTAGE = 65.0  # Punti Elo di vantaggio per la squadra di casa
BASE_K_FACTOR = 24.0   # Costante di reattività base


def calculate_goal_margin_multiplier(goal_diff: int) -> float:
    """
    Calcola il moltiplicatore G basato sul margine di gol,
    ispirato alle formule del World Football Elo Ratings.
    """
    diff = abs(goal_diff)
    if diff <= 1:
        return 1.0
...
class EloEngine:
    """
    Gestisce l'elaborazione cronologica delle partite e il tracciamento
    ...
    def _get_league_files(self) -> List[str]:
        """Recupera tutti i file CSV storici, base e live per la lega specificata."""
    ...
    def load_and_preprocess_matches(self) -> pd.DataFrame:
        """Carica, pulisce e ordina cronologicamente tutte le partite disponibili."""
        ...
        # Rimuovi duplicati (stessa data e stesse squadre) tenendo l'ultimo inserimento
        df = df.drop_duplicates(subset=["Date_Parsed", "HomeClean", "AwayClean"], keep="last")

        # Ordina per data crescente per elaborazione sequenziale temporale corretta
    ...
    def compute_ratings(self) -> Dict[str, float]:
        """
        Calcola i rating Elo iterando cronologicamente su ogni partita.
        """
        ...
        # Inizializza metriche per le squadre
        ...
        for team in all_teams:
            ...
            self.team_stats[team] = {
                ...
                "goals_for": 0,
                "goals_against": 0,
                "peak_elo": DEFAULT_INITIAL_RATING,
                "min_elo": DEFAULT_INITIAL_RATING,
            }

        # Elaborazione partita per partita
    ...
    def get_leaderboard(self) -> pd.DataFrame:
        """Restituisce la classifica Elo ordinata con statistiche e trend recenti."""
        ...
        for team, rating in self.ratings.items():
            ...
            rows.append({
                ...
                "PG": stats.get("matches", 0),
                "V": stats.get("wins", 0),
                "N": stats.get("draws", 0),
                "P": stats.get("losses", 0),
                "GF": stats.get("goals_for", 0),
                "GS": stats.get("goals_against", 0),
                "DR": stats.get("goals_for", 0) - stats.get("goals_against", 0),
                "Delta 5G": round(last_5_delta, 1),
                "Forma 5G": last_5_form,
...
# Caching globale degli engine Elo per ciascun campionato
_ELO_ENGINES_CACHE: Dict[str, EloEngine] = {}


def get_elo_engine(league_name: str) -> EloEngine:
    """Restituisce un'istanza memorizzata nella cache di EloEngine per il campionato specificato."""
...
def get_current_elo(league_name: str) -> Dict[str, float]:
    """
    Restituisce la mappa delle squadre con il loro rating Elo attuale.
    Firma pubblica richiesta: get_current_elo(league_name) -> {team_name: elo_score}
    """
...
def get_elo_leaderboard(league_name: str) -> pd.DataFrame:
    """Restituisce la classifica Elo completa formattata per la visualizzazione."""
    engine = get_elo_engine(league_name)
...
def predict_elo_probs(home_team: str, away_team: str, league_name: str) -> dict:
    """
    Calcola le probabilità attese d'esito (1, X, 2) tra due squadre basate
    sulla differenza Elo con fattore campo.
    Firma pubblica richiesta: predict_elo_probs(home_team, away_team, league_name) -> dict
    """
    engine = get_elo_engine(league_name)
    ...
    # Differenziale con Home Advantage
    dr = r_h + engine.home_adv - r_a
    e_h = 1.0 / (1.0 + 10.0 ** (-dr / 400.0))
    e_a = 1.0 - e_h

    # Modello per probabilità di pareggio (distribuzione gaussiana centrata sullo zero differenziale)
    # Quando dr = 0, il pareggio raggiunge il max ~27%
    p_draw = 0.27 * math.exp(-((dr / 320.0) ** 2))
    p_draw = max(0.06, min(0.34, p_draw))

    # Suddivisione quota rimanente tra vittoria casa e trasferta
    p_home = (1.0 - p_draw) * e_h
    p_away = (1.0 - p_draw) * e_a

    # Normalizzazione per garantire somma esatta = 1.0
    ...
    return {
        ...
        "expected_score_home": round(e_h, 4),
        "expected_score_away": round(e_a, 4),
    }


def get_team_elo_history(team_name: str, league_name: str) -> pd.DataFrame:
    """Restituisce lo storico dell'evoluzione Elo di una singola squadra."""
    engine = get_elo_engine(league_name)
